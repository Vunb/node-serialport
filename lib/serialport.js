...
function SerialPort(path, options, callback) {
  ...
  var push = function(data) {
    debug('push', 'bytes pushed', data.length);
    var ret = this.push(data);
    debug('push', 'returned', ret);
    if (data.length < 25) {
      debug(data.toString());
    }
    return ret;
...
SerialPort.prototype._read = function(bytes) {
  if(!this.isOpen) {
    debug('reading before open, queuing _read');
    this.once('open', function() {
      debug('queued _read(' + bytes + ') after open');
